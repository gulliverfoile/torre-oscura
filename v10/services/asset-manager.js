// ============================================
// asset-manager.js - Gestión unificada de assets (imágenes, sprites, etc.)
// ============================================
...
export const assetManager = {
    ...
    async getAsset(path, type = 'image') {
        ...
        // Si es una URL, cargarla; si no, construir ruta local
        ...
        try {
            ...
            if (type === 'image') {
                ...
                // Otros tipos (audio, etc.) se pueden añadir después
                asset = await fetch(url).then(r => r.blob());
            }
            this.cache.set(path, asset);
            return asset;
        } catch (e) {
            console.warn(`Error cargando asset ${path}, usando placeholder`);
