...
export const explorationManager = {
    ...
    init(rpg) {
        this.rpg = rpg;
        this.core = rpg.core;
        // Sincronizar con configuración del arquitecto
        this.fixedPlayerX = rpg.config?.arch?.player?.x || 300;
        this.fixedPlayerY = rpg.config?.arch?.player?.y || 200;
        this.mapOffsetX = rpg.config?.arch?.minimap?.offsetX || 10;
        this.mapOffsetY = rpg.config?.arch?.minimap?.offsetY || 10;
        this.mapCellSize = rpg.config?.arch?.minimap?.cellSize || 20;
    ...
    entrarHabitacion(y, x) {
        ...
        if (!piso) {
            console.warn('⚠️ Piso no encontrado');
            return false;
        }
        const [maxY, maxX] = piso.tamano;
        if (y < 0 || y >= maxY || x < 0 || x >= maxX) {
            console.log('🚫 Coordenadas fuera del mapa');
            return false;
        }

        const hab = piso.habitaciones.find(h => h.pos[0] === y && h.pos[1] === x);
        if (!hab) {
            console.warn('⚠️ Habitación no encontrada en esas coordenadas');
            return false;
        }
        ...
        if (hab.tipo === 'combate' && !hab.completada) {
            console.log('⚔️ Iniciando combate en esta sala');
    ...
    _intentarMovimiento(dy, dx) {
        ...
        if (this.entrarHabitacion(y, x)) {
            this._moveCooldown = 200; // ms entre movimientos
            console.log(`🚶 Movimiento a [${y},${x}]`);
        }
    },

    draw(ctx) {
        if (!this.salaActual) {
            console.log('⏳ Esperando salaActual...');
            return;
        }
        ctx.fillStyle = '#fff';
    ...
    // Métodos para actualizar configuración desde arquitecto
