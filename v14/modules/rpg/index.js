...
export const rpgModule = {
  ...
  config: {
    ...
    arch: {
      player: { x: 300, y: 250, size: 85 },
      enemy: { x: 600, y: 250, size: 85 },
  ...
  async onEnter() {
    ...
    this.core.on('floor:change-request', (data) => {
      if (this.core.ui && this.core.ui.showDialog) {
        ...
        // Fallback si la UI no está lista
        console.warn('UI no disponible, usando confirm nativo');
        if (confirm('¿Bajar de piso?')) {
          this.exploration.cambiarPiso(data.delta);
        }
      }
    });

    // --- EVENTO DE MUERTE DEL JUGADOR (para game over) ---
    this.core.on('player:dead', (stats) => {
      console.log('[RPG] Evento player:dead recibido', stats);
      if (this.core.ui && this.core.ui.components && this.core.ui.components.gameOver) {
        this.core.ui.components.gameOver.mostrar(stats);
      } else {
        console.warn('[RPG] GameOver component no disponible');
      }
    });
  ...
  update(delta, input) {
    // Actualizar combate o exploración
  ...
  }, // <-- Coma aquí porque después viene renderIngenieria
  ...
  renderIngenieria() {
    ...
    area.innerHTML = `
      ...
      <div id="architect-section" style="margin-top: 20px; border-top: 2px solid #0f0; padding-top: 15px;">
        ${this.core.modules.architect?.getControlesHTML ? this.core.modules.architect.getControlesHTML() : '<p>Módulo arquitecto no disponible</p>'}
      </div>
    `;
    ...
    // Enlazar eventos de los controles de arquitecto si existen
    if (this.core.modules.architect?.bindEvents) {
...
}; // <-- Punto y coma final (cierra export const)